...
int capCheckerArrayList(array_list*);
...
int insertArrayList(array_list* arr, void* val, int index){
    ...
    if(!arr || !val || (index >= arr->size) || (index <= INDEX_OFFSET_UNBOUND)) return STATUS_ERR;
...
int deleteArrayList(array_list* arr, int index){
    if(!arr || (index >= arr->size) || (index <= INDEX_OFFSET_UNBOUND)) return STATUS_ERR;
